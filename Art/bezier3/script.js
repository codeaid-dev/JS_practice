...
const draw = () => {
  ...
  // 制御点start,cp1,cp2,endの補間
  const ax = lerp(start.x, cp1.x, t);
  const ay = lerp(start.y, cp1.y, t);
  const bx = lerp(cp1.x, cp2.x, t);
  const by = lerp(cp1.y, cp2.y, t);
  const cx = lerp(cp2.x, end.x, t);
  const cy = lerp(cp2.y, end.y, t);
  ...
  ctx.fillText(t.toFixed(2), fx, fy-10);

  // ベジェ曲線
  curve.push(new Point(fx,fy));
