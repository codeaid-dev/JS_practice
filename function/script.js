...
console.log(plus(1)); // 101
// 関数式(引数渡し)
function map(f, a) { // const map = (f, a) => {
  let result = [];
...
var num2 = 3;
var name = 'Foo';
function multiply() { // const multiply = () => {
  return num1 * num2;
}
console.log(multiply()); // 60
// 入れ子になっている関数の例
function getScore() { // const getScore = () => {
  var num1 = 2;
  var num2 = 4;
...
// 関数の引数
function myConcat(separator) { // アロー関数はargumentsオブジェクトを持たないため記述できない
  let result = "";
  // 引数について繰り返し
...
// デフォルト引数
function multiply(a, b = 1) { // const multiply = (a, b = 1) => {
  return a * b;
}
console.log(multiply(5,2)); // 10
console.log(multiply(5)); // 5
// 残余引数1
function sum(...theArgs) { // const sum = (...theArgs) {
  let total = 0;
...
// 残余引数2
function countup(a, b, ...next) { // const countup = (a, b, ...next) => {
  console.log(a);
  console.log(b);
  console.log(next);
}
...
sum([1,2,3]);
console.log(sum(...[1,2,3])); // 6 ＊呼出時に...を記述する(スプレッド構文)
// アロー関数
